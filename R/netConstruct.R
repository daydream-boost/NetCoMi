...
#' @param taxRank character indicating the taxonomic rank the network should be 
#'   constructed for. Only used if data (and data 2) is a phyloseq object. 
#'   The given rank must match one of the column names of the taxonomic table 
#'   (the \code{@tax_table} slot of the phyloseq object). If data is a phyloseq 
#'   object and \code{taxRank = NULL}, the row names of the otu table are used 
#'   as node labels.
...
netConstruct <- function(data,
  ...
  if (dataType =="counts") {
    ...
    if("phyloseq" %in% class(data)){
      ...
      if(!is.null(taxRank)){
        if(!taxRank %in% colnames(taxtab)){
          stop('Argument "taxRank" must match column names of the taxonomic table.')
        }
        
        if(any(duplicated(taxtab[, taxRank]))){
          stop(paste0("Taxa names of chosen taxonomic rank must be unique. ",
                      "Consider using NetCoMi's function renameTaxTab()."))
        }
    ...
    if(!is.null(data2) && "phyloseq" %in% class(data2)){
